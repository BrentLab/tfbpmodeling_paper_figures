# figS1C.R
#
# Boxplot of the response rate of the 25 most strongly bound genes (rank_25)
# comparing TCC vs ChIP-chip binding data across the shared regulator set.
#
...
#   plt  <- make_figS1C(ints$harbison, ints$cc)
#
...
# ---------------------------------------------------------------------------
make_figS1C <- function(harbison_rr_intersect, cc_rr_intersect, base_size = 18) {
    bind_rows(harbison_rr_intersect, cc_rr_intersect) %>%
        mutate(binding_source = factor(binding_source,
                                       levels = c("TCC", "ChIP-chip"))) %>%
        ggplot(aes(binding_source, rank_25, fill = binding_source)) +
        geom_boxplot(width = 0.9) +
        xlab("Binding Data Source") +
        ylab("Response rate of 25 most strongly bound genes") +
        scale_y_continuous(
            breaks = seq(0, 1, 0.2),
...
# save_figS1C()
# ---------------------------------------------------------------------------
save_figS1C <- function(path, harbison_rr_intersect, cc_rr_intersect,
                         width = 8, height = 8, bg = "white", ...) {
    plt <- make_figS1C(harbison_rr_intersect, cc_rr_intersect, ...)
...
if (sys.nframe() == 0L) {
    ...
    out_path <- here("plots", data_pull_date, tfbpmodeling_version, "figS1C.svg")
    dir.create(dirname(out_path), recursive = TRUE, showWarnings = FALSE)
    save_figS1C(out_path, ints$harbison, ints$cc)
